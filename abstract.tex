...
Title          & \multicolumn{3}{c|}{Neural network expressiveness} \\ \hline
\end{tabularx}
\end{table}
...
For the purpose of our research we need to introduce various mathematical concepts starting with homology.  Let $X$ be a a topological space. The purpose of homology is to distinguish topological features of $X$. Visually this concept is assimilated to the concept of "holes". Suppose $X$ is of dimension $n \in \N$. The betti numbers of $X$ will be an element of $B \in \N^n$ with $(B_i)_{1 \le i \le n} = \{$The number of $i$-dimensional homology in $X \}$.\\
