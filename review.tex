...
for all $m$. Therefore the fourier transform of $\mu$ is 0 and $\mu$ must be 0. Hence $\sigma$ is discriminatory.
  ...
  In 1991, Hornik extended in \cite{hornik_1991} gave the proof for bounded non-constant functions of $F(I_n,\R)$. The proof is very similar to the original one.\\
...
In 2016 Arora et al. \cite{arora_basu_mianjy_mukherjee_2018} showed a version of the universal approximation theorem for piecewise linear function that includes ReLU functions.\\
...
\begin{proof}
  ...
  \sv
  By theorem 1 in \cite{wang_2005} any piecewise linear function $f: \R^n \to \R$, can be written:\\
  \begin{center}
...
In 2018, Hannin \& Selke \cite{hanin_sellke_2018} showed that the minimal width a ReLUneural network must have in order to approximate any function $f: \R^n \to \R^m$ is $n + m$. Giving a lower bound to the width of the ReLU network.
